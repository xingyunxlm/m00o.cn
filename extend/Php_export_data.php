	...
	public function initialize() {
		...
		switch($this->exportTo) {
			...
			case 'file':
                ...
                $down_path  =   WEB_PATH.'html/download';
                if(!is_dir($down_path)){
                    mkdir($down_path, 777);
                }
                $this->tempFilename =   $down_path.'/'.$this->filename;
                $this->tempFile = fopen($this->tempFilename, "w");
				break;
		}
 ...
 *  by Oliver Schwarz
 */
class Php_export_data extends ExportData {
	...
	private function generateCell($item) {
		...
		if(preg_match("/^-?\d+(?:[.,]\d+)?$/",$item) && (strlen($item) < 15)) {
			$type = 'Number';
            if($item!=0 && strpos($item, '0') === 0 && strpos($item,'.') != 1){//add by zqt,以0开头并且!=0的数据当做为String
                $type = 'String';
            }
		}
